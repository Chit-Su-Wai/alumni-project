...
<body class="bg-slate-50">
            ...
            <div class="hidden lg:block lg:col-span-1 sticky top-20 max-h-[calc(100vh-7rem)] overflow-y-auto scrollbar-hide space-y-2">
                ...
                <div class="bg-white rounded-3xl p-6 shadow-sm border border-cyan-50">
                    <h3 class="font-bold text-lg text-slate-800">
                        <span data-t="showing_posts">Showing</span>
                        <?= $totalPosts ?>
                        <span data-t="posts_label">Posts</span>
                    </h3>
                ...
                <!-- Categories -->
                <div class="bg-white rounded-3xl p-6 shadow-sm border border-cyan-50">
                    <h3 class="font-bold text-lg mb-4 text-slate-800" data-t="categories">
                        Categories
                    </h3>

                    <?php
                    $categories = ["All", "General", "Job", "Event", "News"];
                    foreach ($categories as $cat):
                        ?>
                        <a href="feed.php?category=<?= urlencode($cat) ?>" class="block rounded-xl px-4 py-2.5 mb-2 text-sm font-semibold transition <?= ($category === $cat)
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="bg-white rounded-3xl p-5 shadow-sm border border-cyan-50">
                    <div class="flex items-center gap-3">
                        <img src="<?= htmlspecialchars($currentProfileImage) ?>"
                            class="h-12 w-12 rounded-full object-cover border-2 border-cyan-100">
                        <a href="create_post.php"
                            class="flex-1 rounded-full border border-slate-200 px-5 py-3 text-sm text-slate-400 hover:bg-slate-50 transition"
                            data-t-placeholder="whats_on_mind">What's on your mind?</a>
                    </div>
                </div>
                ...
                <?php foreach ($posts as $post): ?>
                    ...
                    <div id="post-<?= $post['id'] ?>" class="bg-white rounded-3xl p-5 shadow-sm border border-cyan-50">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-400">
                                        <?= date("M d, Y h:i A", strtotime($post['created_at'])) ?></p>
                                </div>
                            </div>
                            <?php if ($post['user_id'] == $_SESSION['user_id']): ?>
                        ...
                        <div class="mt-3">
                            <span
                                class="rounded-full bg-cyan-100 px-3 py-1 text-xs font-semibold text-cyan-700"><?= htmlspecialchars($post['category']) ?></span>
                        </div>

                        <!-- Content -->
                        <div class="mt-3 text-sm text-slate-700 leading-7"><?= nl2br(htmlspecialchars($post['content'])) ?>
                        </div>
                        ...
                        <div class="mt-3 flex items-center gap-4 text-xs text-slate-400">
                            <?php if ($likeCount > 0): ?>
                                <span id="like-info-<?= $post['id'] ?>" class="flex items-center gap-1">
                                    <span
                                        class="flex h-5 w-5 items-center justify-center rounded-full bg-cyan-500 text-[10px] text-white"><i
                        ...
                        <div class="mt-3 border-t border-slate-100 pt-3 grid grid-cols-3 gap-1">
                            <button onclick="toggleLike(<?= $post['id'] ?>)" id="like-btn-<?= $post['id'] ?>"
                                class="flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-semibold transition <?= $isLiked ? 'text-cyan-600 bg-cyan-50' : 'text-slate-500 hover:bg-slate-50' ?>">
                                <i id="like-icon-<?= $post['id'] ?>"
                                    class="<?= $isLiked ? 'fa-solid' : 'fa-regular' ?> fa-thumbs-up"></i>
                                <span data-t="like">Like</span>
                            </button>
                            <button
                                onclick="document.getElementById('comment-section-<?= $post['id'] ?>').classList.toggle('hidden')"
                                class="flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-semibold text-slate-500 hover:bg-slate-50 transition">
                                <i class="fa-regular fa-comment"></i>
                                <span data-t="comment">Comment</span>
                            </button>
                            <button onclick="openShareMenu(<?= $post['id'] ?>)"
                                class="flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-semibold text-slate-500 hover:bg-slate-50 transition">
                        ...
                        <div id="comment-section-<?= $post['id'] ?>" class="hidden mt-3 border-t border-slate-100 pt-3">
                            <form onsubmit="submitComment(event, <?= $post['id'] ?>)" class="flex gap-2">
                                ...
                                <input type="text" id="comment-input-<?= $post['id'] ?>" name="comment"
                                    ...
                                    class="flex-1 rounded-full border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400">
                                <button type="submit"
                                    class="rounded-full bg-gradient-to-r from-cyan-400 to-teal-500 px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:shadow transition">
            ...
            <div class="hidden lg:block lg:col-span-1 sticky top-20 max-h-[calc(100vh-7rem)] overflow-y-auto scrollbar-hide space-y-2">
                ...
                <div class="bg-white rounded-3xl p-5 shadow-sm border border-cyan-50">
                    <h3 class="font-bold text-lg mb-3 text-slate-800">
                        <i class="fa-regular fa-calendar mr-2 text-teal-600"></i>
                        <span data-t="calendar">Calendar</span>
                    </h3>
                ...
                <div class="bg-white rounded-3xl p-5 shadow-sm border border-cyan-50">
                    <div class="flex items-center justify-between mb-3">
                        <h3 class="font-bold text-lg text-slate-800">
                            <i class="fa-regular fa-newspaper mr-2 text-teal-600"></i>
                            <span data-t="recent_posts">Recent Posts</span>
                        </h3>
                        <span id="recentDateLabel" class="text-xs text-teal-600 font-semibold hidden"></span>
                    </div>

                    <div id="recentPostsList">
                    <?php if ($recent && $recent->num_rows > 0): ?>
                        <?php while ($r = $recent->fetch_assoc()): ?>
                            <a href="feed.php#post-<?= $r['id'] ?>"
                                class="flex items-center gap-3 mb-3 pb-3 border-b border-slate-100 last:border-0 last:mb-0 last:pb-0 hover:bg-cyan-50/50 rounded-lg px-2 py-1 -mx-2 transition">

                                <img src="<?= !empty($r['profile_image']) ? htmlspecialchars($r['profile_image']) : '../images/default-avatar.svg' ?>"
                                    class="w-8 h-8 rounded-full object-cover border border-cyan-100">

                                <div class="flex-1 min-w-0">
                                    <div class="text-sm font-medium text-slate-700 line-clamp-1">
                                        <?= htmlspecialchars(substr($r['content'], 0, 40)) ?>...
                                    </div>
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <p class="text-sm text-slate-400">No posts in the last 3 days.</p>
                    <?php endif; ?>
                    </div>
        ...
        <div class="bg-white rounded-3xl p-6 w-80 shadow-2xl">
            ...
            <div class="space-y-3">
                ...
                <button onclick="shareFacebook()"
                    ...
                    data-t="facebook"><i class="fa-brands fa-facebook-f"></i> Facebook</button>
    ...
    <script>
        ...
        /* Calendar Widget with date-filter for Recent Posts */
        (function () {
            ...
            function renderCalendar() {
                ...
                for (var d = 1; d <= daysInMonth; d++) {
                    ...
                    var cls = 'py-1 rounded-lg cursor-pointer transition ';
                    if (isSelected) {
                        cls += 'bg-cyan-500 text-white font-bold ring-2 ring-cyan-300';
                    } else if (isToday) {
                        cls += 'bg-teal-500 text-white font-bold';
                    } else {
                        cls += 'text-slate-600 hover:bg-cyan-50';
                    }

                    html += '<div class="' + cls + '" onclick="calSelectDate(\'' + dateStr + '\')">' + d + '</div>';
                }
                html += '</div>';
                cal.innerHTML = html;
            }

            /* Fetch recent posts (by date or default 3 days) and update sidebar */
            function loadRecentPosts(dateStr) {
                var url = 'api_posts_by_date.php' + (dateStr ? '?date=' + dateStr : '');
                ...
                fetch(url)
                    ...
                    .then(function (data) {
                        if (!data.posts || data.posts.length === 0) {
                            list.innerHTML = '<p class="text-sm text-slate-400">' +
                                (dateStr ? 'No posts found for this date.' : 'No posts in the last 3 days.') + '</p>';
                            return;
                        }

                        var html = '';
                        data.posts.forEach(function (p) {
                            html += '<a href="feed.php#post-' + p.id + '" ' +
                                'class="flex items-center gap-3 mb-3 pb-3 border-b border-slate-100 last:border-0 last:mb-0 last:pb-0 hover:bg-cyan-50/50 rounded-lg px-2 py-1 -mx-2 transition">' +
                                '<img src="' + p.profile_image + '" class="w-8 h-8 rounded-full object-cover border border-cyan-100">' +
                                '<div class="flex-1 min-w-0">' +
                                '<div class="text-sm font-medium text-slate-700 line-clamp-1">' + p.content + '...</div>' +
                                '<div class="text-xs text-slate-400 mt-0.5">' + p.date_label + '</div>' +
                                '</div></a>';
                        });
                        list.innerHTML = html;

                        if (dateStr) {
                            /* Format label e.g. "Jun 30" */
                            var parts = dateStr.split('-');
                            var labelDate = new Date(parts[0], parts[1] - 1, parts[2]);
                            var mo = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
                            label.textContent = mo[labelDate.getMonth()] + ' ' + labelDate.getDate();
                    ...
                    .catch(function () {
                        list.innerHTML = '<p class="text-sm text-red-400">Failed to load posts.</p>';
                    });
            }
